...
class NewUploadParserAutomatonTests(unittest.TestCase):
    ...
    def __setup_for_parse_with_percentile(self, expect_percent_calc=True):
        self.mox.StubOutWithMock(recalc_util, "recalculate_percentile_raw")

        self.__setup_test_cdi(False)

        if expect_percent_calc:
            recalc_util.recalculate_percentile_raw(
                self.__adapter,
                "test_mcdi_type",
                constants.MALE,
                3,
    ...
    def test_parse_full_record_valid_provided_percentile(self):
        self.__setup_for_parse_with_percentile()
    ...
    def test_parse_full_record_invalid_num_languages(self):
        self.__setup_for_parse_with_percentile(False)
    ...
    def test_parse_full_record_valid_not_provided_session_num(self):
        self.mox.StubOutWithMock(recalc_util, "get_session_number")

        recalc_util.get_session_number(
            "test_study",
            "test_study_id"
        ).AndReturn(11)
